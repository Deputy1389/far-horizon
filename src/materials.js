...
async function loadLocalSwgTextures(prep){
  ...
  await Promise.all(entries.map(([role,descriptor])=>new Promise(resolve=>{
    ...
    // string form so local manifests generated by an earlier checkout still
    // work without regenerating the client assets.
    const url=typeof descriptor==='string'?descriptor:descriptor?.url;
...
export async function createMaterialLibrary(renderer){
  ...
  const wallDetail=local?.wallDetail||null;
  const concrete=local?.concrete||null;
  const concreteDetail=local?.concreteDetail||null;
  const roadMap=local?.road||concrete||floor||null;
  const roadDetailMap=local?.roadDetail||concreteDetail||null;
  const padMap=local?.pad||roadMap||floor||null;
  const metalMap=local?.metal||metal;
  const metalVentMap=local?.metalVents||metalMap;
